...
void I2C0_Init(void){
  ...
  // if the SMCLK is set to 12 MHz, divide by 30 for 400 kHz baud clock
  UCB1BRW = 30;
// P6SEL0 |= 0xC0;                   //
...
*/
s8 I2C0_bus_read(u8 dev_addr, u8 reg_addr, u8 *reg_data, u8 cnt)
{
   s32 iError = 0;
   uint8_t i;
   ...
   while(UCB1STATW&0x0010){
       if(g_ticks - ticks >= 100){
           ...
           I2C0_Init();                    // reset to known state
           return iError=-1;
       }
   };         // wait for I2C ready
   UCB1CTLW0 |= 0x0001;               // hold the eUSCI module in reset mode
 ...
 */
s8 I2C0_bus_write(u8 dev_addr, u8 reg_addr, u8 *reg_data, u8 cnt)
{
    s32 iError = 0;
    u8 array[8];
    u8 stringpos = 0;
